...
switch($_GET[PHPIdSession]){
  ...
  ?>   
      <tr>
        <td class="cc">Institusi</td>   <td colspan=2 class="cb"> : <select name="institusi" onChange="MM_jumpMenu('parent',this,0)">
          <option value="media.php?page202055ID=7352cad8519dda2be442fca9be8e914d">- Pilih Institusi -</option>
      <?php
      ...
      if ($d['Identitas_ID']==$id){
        ...
        echo "<option value='?page=akademiktahun&codd=$d[Identitas_ID]' $cek> $d[Identitas_ID] - $d[Nama_Identitas]</option>";
  		}
  		?>
        </select>
      ...
      <tr>
        <td class="cc">Jurusan</td>   <td colspan=2 class="cb"> : <select name="jurusan" onChange="MM_jumpMenu('parent',this,0)">
          <option value="media.php?page=akademiktahun">- Pilih Jurusan -</option>
      <?php
      ...
      if ($d1['kode_jurusan']==$kode){
        ...
        echo "<option value='?page=akademiktahun&codd=$id&kode=$d1[kode_jurusan]' $cek> $d1[kode_jurusan] - $d1[nama_jurusan]</option>";
  		}
  		?>
        </select>
      ...
      <tr>
        <td class="cc">Program</td>   <td colspan=2 class="cb"> : <select name="program" onChange="MM_jumpMenu('parent',this,0)">
          <option value="media.php?page=akademiktahun">- Pilih Program -</option>
      <?php
      ...
      if ($d2['ID']==$idp){
        ...
        echo "<option value='?page=akademiktahun&codd=$id&kode=$kode&id=$d2[ID]' $cek> $d2[Program_ID] - $d2[nama_program]</option>";
  		}
  		?>
        </select>
        <input name="TxtKodeH" type="hidden" value="<?= $idp; ?>"></td>
      </tr>
      <tr><td class="cc">Pilihan </td> <td colspan=2 class="cb"> : <a class="s" href="?page=akademiktahun&PHPIdSession=tambahtahun">Tambah Tahun Akademik</a> </tr></td>
      <?php
          echo" 					
          	<table id=tablemodul>
            ...
            $sql="SELECT t1.*,t2.nama_jurusan FROM tahun t1,jurusan t2 WHERE t1.Jurusan_ID=t2.kode_jurusan AND t1.Jurusan_ID='$kode' AND t1.Program_ID='$idp'";
          	$qry= mysql_query($sql)
          	or die ();
            ...
            if($r[Aktif]=='Y'){
          	...
          	$no++;
               echo "<tr bgcolor=$warna>                            
                     <td>$no</td>
        		         <td><a class=s href='?page=akademiktahun&PHPIdSession=chagta&gis=$r[ID]'>$r[Tahun_ID] <img src=../img/edit.png></a></td>
        		         <td>$r[Nama]</td>
  ...
  $r=mysql_fetch_array($e);
     ...
     echo"<div id=groupmodul1>               
          <form method=post action=akademik/akademiktahunact.php?page=akademiktahun&PHPIdSession=upta>
 <input type=hidden name='ID' value='$r[ID]'>         
  <h3>Edit Tahun Akademik</h3> <table id=tablemodul>
      <tr><td class=cc>Institusi :</td>   <td colspan=2 class=cb><select name=cmi2 id=cmi2>
        <option value=0>- Pilih Institusi -</option>";
                                      $tampil=mysql_query("SELECT * FROM identitas ORDER BY ID");
    ...
    <tr><td class=cc>Jurusan :</td>   <td colspan=2 class=cb><select name=cmbj2 id=cmbj2>
    ...
    <option value=>Pilih Program</option>";
                                      $tampil=mysql_query("SELECT * FROM program WHERE Identitas_ID='$r[Identitas_ID]' ORDER BY ID");
                                      while($w=mysql_fetch_array($tampil)){
  ...
  case "tambahtahun":
    ...
    echo"<div id=groupmodul1>
      ...
      <form action='akademik/akademiktahunact.php?page=akademiktahun&PHPIdSession=addta' method='post'>
      ...
      ?>
        <tr>
          <td class="cc">Institusi *</td>   <td colspan=2> <select name="cmi1" onChange="MM_jumpMenu('parent',this,0)">
            <option value="media.php?page=akademiktahun&PHPIdSession=tambahtahun">- Pilih Institusi -</option>
        <?php
    	  $sqlp="SELECT * FROM identitas ORDER BY Identitas_ID";
    	  $qryp=mysql_query($sqlp)
    	  or die();
        ...
        if ($d['Identitas_ID']==$id){
          ...
          echo "<option value='?page=akademiktahun&PHPIdSession=tambahtahun&codd=$d[Identitas_ID]' $cek> $d[Identitas_ID] - $d[Nama_Identitas]</option>";
    		}
    		?>
          </select>
        ...
        <tr>
          <td class="cc">Jurusan *</td>   <td colspan=2> <select name="cmbj1" onChange="MM_jumpMenu('parent',this,0)">
            <option value="media.php?page=akademiktahun&PHPIdSession=tambahtahun">- Pilih Jurusan -</option>
        <?php
    	  $sqlp="SELECT * FROM jurusan WHERE Identitas_ID='$id'";
    	  $qryp=mysql_query($sqlp)
    	  or die();
        ...
        if ($d1['kode_jurusan']==$kode){
          ...
          echo "<option value='?page=akademiktahun&PHPIdSession=tambahtahun&codd=$id&kode=$d1[kode_jurusan]' $cek> $d1[kode_jurusan] - $d1[nama_jurusan]</option>";
    		}
    		?>
          </select>
          <input name="cmbj1" type="hidden" value="<?= $kode; ?>"></td>
        </tr>
        <?echo"
        <tr><td class=cc>Program Jawab *</td>    <td>      <select name='cmbp1'>
            <option value=0 selected>- Pilih Program -</option>";
                  $tampil=mysql_query("SELECT * FROM program WHERE Identitas_ID='$id' ORDER BY ID");
